...
mov rcx, 0
mov r10, 6
mov rbx, %1				;loads string into rbx
%%convert6to10:
mov r8b, [rbx+rcx]		;moves first character into r8b
inc rcx
...
mov rcx, 0
mov r10, 6
mov rbx, aSenaryLength	;loads string into rbx
convert6to10:
mov r8b, [rbx+rcx]		;moves first character into r8b
inc rcx
cmp r8b, 0x20			;compares to space
...
add al, r8b				;adds value into rax
mov r9b, [rbx+rcx]
cmp r9b, 0				;check if next char is null
je exit					;exits loop if it is
mul r10w				;multiplies by 6 if it isn't
...
printLoop:
	...
	push	rsi
	push	r12
	mov	eax, dword [perimsArray+rsi*4]
...
mov	eax, dword [perimsArray]
	...
	mov	dword [perimSum], 0
	mov	rcx, 0
	mov	ecx, dword [length]
	mov	rdi, 0
sumLoop:
	mov	rax, 0
	mov	eax, dword [perimsArray+rdi*4]
	add	dword [perimSum], eax
	cmp	eax, dword [perimMin]
	jge	notNewMin
	mov	dword [perimMin], eax
notNewMin:
	cmp	eax, dword [perimMax]
	jle	notNewMax
	mov	dword [perimMax], eax
notNewMax:
	inc	rdi
...
inc rcx				;counts how many loops there is
cmp eax, 0
jne convert6		;loops if the quotient is not 0
mov rbx, 11
sub rbx, rcx		;spaces to right justify the string
mov rcx, 0	
rightJust:			;loop to right justify
mov [r8], byte ' '	;add spaces
inc r8
inc rcx
cmp rbx, rcx
jne rightJust
popLoop:			;loop to pop numbers off the stack
inc rcx
pop rax
add rax, 0x30
mov [r8], rax
