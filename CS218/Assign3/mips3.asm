...
#  Section: 1004
#  Assignment: 3
#  Description:  find the areas of trapezoids
...
#	YOUR CODE GOES HERE
move $t0, $a1
remu $t0, $t0, 2		# find if even or odd
beqz $t0, evenMed

# odd estMed
move $t0, $a1
divu $t0, $t0, 2		# len / 2
mul $t0, $t0, 4			# word sized
add $a0, $a0, $t0		# middle address
lw $v0, ($a0)
...
lw $t0, ($a0)
lw $t1, ($a1)
addu $t0, $t0, $t1				# asides + csides
divu $t0, $t0, 2				# ^ / 2
lw $t1, ($a2)
mul $t0, $t0, $t1				# ^ * height
lw $t1, ($sp)					# pull address out of stack
sw $t0, ($t1)
addu $a0, $a0, 4				# inc addresses
addu $a1, $a1, 4
addu $a2, $a2, 4
add $t1, $t1, 4
sw $t1, ($sp)
sub $a3, $a3, 1					# loop exit condition
bnez $a3, areasLoop
...
#	YOUR CODE GOES HERE
li $t0, FALSE				# $t0 = sorted variable
sub $a1, $a1, 1				# length - 1

sortLoop:
beq $t0, TRUE, sortExit
li $t0, TRUE
move $t2, $a0				# list into $t2

# odd sort
li $t1, 1					# i = 1
add $t2, $t2, 4				# inc list to list[1]
oddLoop:
bge $t1, $a1, evenStart		# exit condition
lw $t3, ($t2)				# list[i]
add $t2, $t2, 4				
lw $t4, ($t2)				# list[i+1]
add $t2, $t2, 4
add $t1, $t1, 2
bgt $t3, $t4, sortOdd		# if list[i]>list[i+1]
j oddLoop
...
move $t2, $a0
evenLoop:
bge $t1, $a1, sortLoop		# restart loop
lw $t3, ($t2)
add $t2, $t2, 4				
lw $t4, ($t2)
add $t2, $t2, 4
add $t1, $t1, 2
bgt $t3, $t4, sortEven		# if list[i]>list[i+1}]
j evenLoop

sortExit:
jr $ra

sortOdd:					# sorting code
sub $t2, $t2, 8
sw $t4, ($t2)				# move [i+1] back
add $t2, $t2, 4
sw $t3, ($t2)				# move [i] forward
add $t2, $t2, 4
li $t0, FALSE
j oddLoop
...
mtc1 $t1, $f0
cvt.s.w $f0, $f0
s.s $f0, ($a2)			# convert to float
mtc1 $a1, $f1
cvt.s.w $f1, $f1
div.s $f0, $f0, $f1		# div bu length
s.s $f0, ($a3)
