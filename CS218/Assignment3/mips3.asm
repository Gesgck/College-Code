...
#  Section: 1004
#  Assignment: MIPS #3
#  Description: MIPS stack and procedure calling convention.
...
move $t4, $zero
sub $t2, $t2, 1				# sub 1 from length

bnez $t2, areaLoop			# loop condition
jr $ra
...
sub $t4, $t4, 1				# j = i-1
whileLoop:
blez $t4, insert			# while (j>_0)
lw $t6, ($t5)				# value of arr[j] in $t6
bgtu $t6, $t3, insert		# arr[j] < value
...
move $t0, $a0				# array address in $t0
move $t1, $a1				# length in $t1
move $t2, $zero
...
mtc1 $v0, $f2
cvt.s.w $f2, $f2			# convert sum to float
lw $t0, ($sp)
mtc1 $t0, $f3
cvt.s.w $f3, $f3			# convert length to float

div.s $f2, $f2, $f3			# div by float length
mov.s $f0, $f2
...
subu $sp, $sp, 24			# saving args on stack
sw $s0, ($sp)
sw $s1, 4($sp)
sw $s2, 8($sp)
sw $s3, 12($sp)
...
jal insertionSort			# insertionSort call

move $t4, $s0				# set up args for min, med, and max
move $t1, $s1
move $t3, $zero
...
subu $t4, $t4, 4
lw $t0, ($t4)
addu $t3, $t3, $t0			# add two middle areas
divu $t3, $t3, 2			# div middle areas by 2
...
move $a0, $s0				# iSum call
move $a1, $s1
jal iSum
move $t0, $s2
sw $v0, ($t0)

move $a0, $s0				# iAverage call		
move $a1, $s1
jal iAverage
lw $t0, 8($fp)
sw $v0, ($t0)

move $a0, $s0				# fAverage call	
move $a1, $s1
jal fAverage
lw $t0, 12($fp)
s.s $f0, ($t0)
...
move $s0, $a0				# address in $s0
move $s1, $a1				# length in $s1
move $s2, $a2				# sum in $s2
move $s3, $a3				# min in $s3
...
dispalyLoop:				# display start
la $a0, spc					# print space
li $v0, 4
syscall

lw $a0, ($s0)				# print number
li $v0, 1
syscall
...
beqz $s1, displayExit		# exit condition

beqz $s4, newlinePrint		# new line condition
j dispalyLoop
